...
<body>
    <div class="container mt-5 ">
        ...
        <div>
            ...
            <div class="border-dotted border-top-0 border-right-0 border-left-0 mt-3"><strong class="">Invoice
                    Receipt</strong></div>
            <div class="fa-3x">------------------</div>
            ...
            <div class="">
                @if ($application->travelAgent)
                    <div class="border-dotted border-top-0 border-right-0 border-left-0 mt-4">
                        <strong>Travel agent: <span
                                style="font-size: 27px">{{ $application->travelAgent->name }}</span></strong>
                    </div>
                    <div class="fa-3x">---------------</div>
                    <div>Account number: {{ $application->travelAgent->account_number }}</div>

                    <div>Name: <span
                            style="font-size: 27px">{{ $application->first_name . ' ' . $application->last_name }}</span>
                    </div>
                @else
                    Direct
                @endif
            ...
            @endphp
            <div class="font-weight-bolder" style="font-weight: bolder">Total Fees : {{ $total }} USD
                ({{ $application->payment_method }})</div>
            <div>Fees in words : {{ \App\Helpers\convertNumberToWorldsInUsd($total) }} </div>
            <div class="mt-2">
                <p>{{ $settings->invoice_footer }}</p>
            </div>
            @if (\App\Helpers\isExistVat())
                <p>VAT Regt : {{ \App\Helpers\registrationNumber() }}</p>
            @endif
        </div>
