            ...
            <div class="row">
                ...
                @php
                    ...
                    foreach ($agents->get() as $agent) {
                        ...
                        $applicationSum =
                            $agent->applications()->sum('vat') +
                            $agent->applications()->sum('dubai_fee') +
                            $agent->applications()->sum('service_fee');

                        // Sum for serviceTransactions
                        $serviceTransactionSum =
                            $agent->serviceTransactions()->where('status', '!=', 'deleted')->sum('vat') +
                            $agent->serviceTransactions()->where('status', '!=', 'deleted')->sum('dubai_fee') +
                            $agent->serviceTransactions()->where('status', '!=', 'deleted')->sum('service_fee');
                    ...
                    $dataDirect = \Illuminate\Support\Facades\DB::table(
                        \Illuminate\Support\Facades\DB::raw('(SELECT name, surname, vat, service_fee, dubai_fee FROM service_transactions WHERE status != "deleted" AND agent_id IS NULL
                    UNION ALL
                    SELECT first_name AS name, last_name AS surname, vat, service_fee, dubai_fee FROM applications WHERE travel_agent_id IS NULL) AS combined_data'),
                        ...
                        ->select(
                            ...
                            \Illuminate\Support\Facades\DB::raw(
                                'SUM(vat + service_fee + dubai_fee) AS total_combined_fee',
                            ),
                        )
                        ...
                        <tbody>
                            @foreach ($data['agents'] as $record)
                                <tr>
                                    ...
                                    <td class='text-center'>$ {{ \App\Helpers\formatCurrency($record['total_sales']) }}
                                    </td>
                                    <td class='text-center'>$
                                        {{ \App\Helpers\formatCurrency($record['un_paid_bail']) }}</td>
                                </tr>
                            @endforeach
                        <tfoot>
                            <tr>
                                ...
                                <td class="text-center">
                                    {{ \App\Helpers\formatCurrency($data['total_sales_for_all_agents']) }}</td>
                                <td class="text-center">
                                    {{ \App\Helpers\formatCurrency($data['total_un_paid_bal_for_agents']) }}</td>
                            </tr>
                        </tfoot>
                        </tbody>
                    </table>
                        ...
                        <tbody>
                            @foreach ($data['directs'] as $record)
                                <tr>
                                    ...
                                    <td class='text-center'>$ {{ \App\Helpers\formatCurrency($record['un_paid']) }}
                                    </td>
                                </tr>
                            @endforeach
                        <tfoot>
                            ...
                            <tr>
                                <td colspan="2">Total </td>
                                <td class="text-center">
                                    {{ \App\Helpers\formatCurrency($data['total_sales_for_direct']) }}</td>
                                <td class="text-center">
                                    {{ \App\Helpers\formatCurrency($data['total_un_paid_bal_for_direct']) }}</td>
                            </tr>
                            <tr>
                                <td></td>
                                <td></td>
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    {{ \App\Helpers\formatCurrency($data['total_sales_for_direct'] + $data['total_sales_for_all_agents']) }}
                                </td>
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    {{ \App\Helpers\formatCurrency($data['total_un_paid_bal_for_agents'] + $data['total_un_paid_bal_for_direct']) }}
                                </td>
                                <td class="text-center"></td>
                            </tr>
                        </tfoot>
